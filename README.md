...
<p align="center">
  <img src="assets/preview.jpg" alt="Game UI Preview" width="800" />
</p>
...
### 1. Card Merging and Collision Interaction
Drag a number card over another card and press the operation keys (W/A/S/D) to merge them. The preview highlights the target card and shows the active operator.
<p align="center">
  <img src="assets/game_play.png" alt="Card Merging and Collision Interaction" width="600" />
</p>

### 2. Level Settlement and Solutions Preview
When the game ends (due to timeout or stamina depletion), the settlement dialog shows the reference solutions generated by the uniqueness solver.
<p align="center">
  <img src="assets/game_failed.png" alt="Level Settlement and Solutions Preview" width="600" />
</p>

### 3. Game History and PDF Export
Review your detailed game logs of current session and export them as a clean PDF document for printing or archiving.
<p align="center">
  <img src="assets/print_history.png" alt="Game History and PDF Export" width="600" />
</p>
...
This project was developed around February 2026, during the peak popularity of Gemini 3.0 Pro. The user interface (UI) was iteratively adjusted through vibe coding, and the algorithms were designed using a results-oriented approach explored in collaboration with AI. Neither the UI nor the algorithms were created in a single run; they are the result of iterative refinement and are provided for reference only.
...
### 1. 卡片拖拽碰撞与合并交互
将一个数字卡片拖拽到另一个卡片上，在拖拽过程中按下操作按键（W/A/S/D）即可合并它们。系统会高亮目标卡片并显示当前激活的运算符。
<p align="center">
  <img src="assets/game_play.png" alt="卡片拖拽碰撞与合并交互" width="600" />
</p>

### 2. 关卡结算与答案提示
当游戏结束时（因时间耗尽或体力扣完），结算弹窗会展示由唯一解求解器计算出的所有参考答案。
<p align="center">
  <img src="assets/game_failed.png" alt="关卡结算与答案提示" width="600" />
</p>

### 3. 历史记录与 PDF 导出
查看当前会话的详细答题记录，并可调用浏览器打印功能将其导出为排版整洁的 PDF 文档。
<p align="center">
  <img src="assets/print_history.png" alt="历史记录与 PDF 导出" width="600" />
</p>
...
本项目开发于 2026 年 2 月左右，正值 Gemini 3.0 Pro 风靡之时。用户界面（UI）是通过 vibe coding 进行持续微调与迭代优化的，算法逻辑则是以结果为导向、与 AI 共同探索设计而成的。UI 和算法并非一次性生成，而是多次迭代打磨的结果，在此仅供参考。
